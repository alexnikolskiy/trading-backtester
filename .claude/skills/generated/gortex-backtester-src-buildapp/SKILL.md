...
| File | Symbols |
|------|---------|
| `apps/backtester/src/app.ts` | reap, config, timer, dispose, drain, ... |
| `apps/backtester/src/config.ts` | AppConfig, overlayImage, overlayPolicy, env, loadConfig |
| `apps/backtester/src/index.ts` | config, addr, main, shutdown, shutdown, ... |
